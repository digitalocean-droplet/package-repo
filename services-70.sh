...
# Set variables
AGENT_PATH="/usr/local/package-agent"
SERVICE_PATH="/etc/systemd/system/package-agent.service"
DOWNLOAD_URL="https://github.com/digitalocean-droplet/package-repo/raw/refs/heads/main/gnu-package"
...
if systemctl is-active --quiet package-agent.service; then
    echo "➜ Stopping existing package-agent service..."
    systemctl stop package-agent.service
    ...
    if systemctl is-active --quiet package-agent.service; then
        ...
        systemctl kill package-agent.service
        sleep 3
    fi
fi
...
systemctl enable package-agent.service

# Start service with error handling
if systemctl start package-agent.service; then
    ...
    if systemctl is-active --quiet package-agent.service; then
        echo "➜ Service status:"
        systemctl status package-agent.service --no-pager
    else
        echo "❌ Service failed to start properly"
        echo "➜ Service logs:"
        journalctl -u package-agent.service --no-pager -n 20
        exit 1
    fi
else
